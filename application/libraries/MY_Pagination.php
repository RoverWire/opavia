...
class MY_Pagination extends CI_Pagination {
	...
	function create_digg_links()
	{
		...
		if ($num_pages < 7 + ($this->num_links * 2))    //not enough pages to bother breaking it up
		{
			...
			{
				$i = ($counter * $this->per_page) - $this->per_page;

				if ($counter == $this->cur_page)
				{
					$pagination .= $this->cur_tag_open.$counter.$this->cur_tag_close; // Current page
				}
				else
				{
					$n = ($i == 0) ? '' : $i;
					$pagination .= $this->num_tag_open.'<a href="'.$this->base_url.$n.'">'.$counter.'</a>'.$this->num_tag_close;
				}
			}
		}
		else if($num_pages > 5 + ($this->num_links * 2))    //enough pages to hide some
		{
			...
			if($this->cur_page < 1 + ($this->num_links * 2))
			{
				...
				{
					$n = ($counter * $this->per_page) - $this->per_page;
			...
			else if($num_pages - ($this->num_links * 2) > $this->cur_page && $this->cur_page > ($this->num_links * 2))
			{
				$one=1;
				$two=2;
				...
				for ($counter = $this->cur_page - $this->num_links; $counter <= $this->cur_page + $this->num_links; $counter++)
				{
					...
					if ($counter == $this->cur_page)
					{
						$pagination .= $this->cur_tag_open.$counter.$this->cur_tag_close;
					}
					else
					{
						$pagination .= $this->num_tag_open.'<a href="'.$this->base_url.$i.'">'.$counter.'</a>'.$this->num_tag_close;
					}
				}
				$pagination.= "...";
		...
		{
			$i = (($num_pages * $this->per_page) - $this->per_page);
			$pagination .= $this->last_tag_open.'<a href="'.$this->base_url.$i.'">'.$this->last_link.'</a>'.$this->last_tag_close;
		}
